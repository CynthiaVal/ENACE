...
$enlace=Conec_db();
?>
<p align="center" class="Estilo1">Expediente ENACE  </p>
<p>
<?php
	  	
		$idenace = $_POST['id'];
	  	$Nro_caja = $_POST['txtCaja'];		
		$Nro_file = $_POST['txtFile'];
		$Programa = $_POST['cmbPro'];
		$Titular =utf8_decode($_POST['txtTitular']);
		$Titular = strtoupper($Titular);
		$Manzana = $_POST['txtMzn']; //saca el id d	
		$Lote= $_POST['txtLote']; //saca el id  
		$Nro_folios = $_POST['txtFolios'];
	
		//tipo de escritura
		include("../../Clases/clsPrograma.php");
		$ClsPrograma = new clsPrograma;
		$Programa=$ClsPrograma->fncSelProgramaNom($Programa);
		...
		$ClsEnace->fncUpdEnace($idenace,$Nro_caja, $Nro_file, $Programa[0],$Titular,$Manzana,$Lote, $Nro_folios); 
	  ?>	        
            </p>
	<!-- InstanceEndEditable -->					 					
